...
Open another terminal, and run `tfenv -v` to check if the installation works. Finally, install Terrafrom version you required using command `tfenv install x.x.x`. In this repo, I use Terraform version `1.14.0`, so run below command to install `1.14.0`.

```bash
tfenv install 1.14.0

# Switch to use 1.14.0
tfenv use 1.14.0
